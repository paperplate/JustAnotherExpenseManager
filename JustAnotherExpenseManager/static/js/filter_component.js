//#region static_src/js/filter_component.ts
function selectCategory(li) {
	...
	if (isAll) {
		...
		if (!allItems.some((el) => el.dataset.value !== "" && el.classList.contains("selected"))) allLi?.classList.add("selected");
	}
...
function updateCategorySummary() {
	...
	const summaryText = selected.length === 0 ? "All Categories" : selected.length === 1 ? selected[0].dataset.value ?? "All Categories" : `${selected.length} categories`;
...
function updateTagSummary() {
	...
	const summaryText = selected.length === 0 ? "All Tags" : selected.length === 1 ? selected[0].dataset.value ?? "All Tags" : `${selected.length} tags`;
...
async function loadCategories() {
	try {
		...
		categories.forEach((cat) => {
			...
			if (selectedCategories.includes(cat.category_name)) li.classList.add("selected");
			list.appendChild(li);
		});
		if (selectedCategories.length > 0) {
			document.querySelector("#category-details .filter-option[data-value=\"\"]")?.classList.remove("selected");
			updateCategorySummary();
...
async function loadTags() {
	try {
		...
		tags.filter((tag) => !tag.startsWith("category:")).forEach((tag) => {
			const li = document.createElement("li");
			li.className = "filter-option";
			li.dataset.value = tag;
			li.textContent = tag;
			li.onclick = () => selectTag(li);
			if (selectedTags.includes(tag)) li.classList.add("selected");
			list.appendChild(li);
		});
		if (selectedTags.length > 0) {
			document.querySelector("#tag-details .filter-option[data-value=\"\"]")?.classList.remove("selected");
			updateTagSummary();
...
function filterByTimeRange(range) {
	...
	if (range === "custom") {
		...
		return;
	} else customPicker.style.display = "none";
	applyFilters();
}
...
document.addEventListener("DOMContentLoaded", () => {
	...
	if (window.location.search) {
		...
		if (targetUrl && targetElement) fetch(targetUrl + window.location.search).then((response) => response.text()).then((html) => {
			const target = document.querySelector(targetElement);
			if (target) target.innerHTML = html;
